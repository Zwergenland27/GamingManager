...
public class EmailVerificationRequestedEventHandler : INotificationHandler<EmailVerificationRequestedEvent>
{
	...
	{
		var link = new Uri($"https://localhost:5001/api/Users/{notification.Username.Value}/Email/verify/{notification.VerificationToken.Token}");
		Console.WriteLine($"Send email to user: {notification.Email.Value} Greetings, {notification.Username.Value}! Your verification link is the following: {link}");
